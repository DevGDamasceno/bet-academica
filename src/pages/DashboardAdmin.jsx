...
const respostaApostas = await
fetch(`http://localhost:3000/apostas?eventoId=${eventoId}`);
...
const resUser = await
fetch(`http://localhost:3000/usuarios/${aposta.usuarioId}`);
...
<header style={{ display: 'flex', justifyContent: 'space-between', alignItems:
'center', marginBottom: '30px' }}>
<h1>Painel do Administrador</h1>
<div>
<span style={{ marginRight: '15px' }}>Olá, {usuario?.nome}</span>
<button onClick={logout} style={{ width: 'auto', backgroundColor: '#e74c3c'
}}>Sair</button>
</div>
</header>
...
<div className="login-box" style={{ flex: '1', minWidth: '300px', margin: '0',
height: 'fit-content' }}>
<h3>Cadastrar Nova Partida</h3>
<form onSubmit={cadastrarEvento} style={{ marginTop: '15px' }}>
<input type="text" placeholder="Time da Casa (A)" value={timeA}
onChange={(e) => setTimeA(e.target.value)} required />
<input type="text" placeholder="Time Visitante (B)" value={timeB}
onChange={(e) => setTimeB(e.target.value)} required />
<input type="date" value={dataEvento} onChange={(e) =>
setDataEvento(e.target.value)} required />

<h4 style={{ margin: '15px 0 5px' }}>Definir Odds</h4>
<div style={{ display: 'flex', gap: '10px' }}>
<input type="number" step="0.01" placeholder="Vitória A" value={oddA}
onChange={(e) => setOddA(e.target.value)} required />
<input type="number" step="0.01" placeholder="Empate"
value={oddEmpate} onChange={(e) => setOddEmpate(e.target.value)} required />
<input type="number" step="0.01" placeholder="Vitória B" value={oddB}
onChange={(e) => setOddB(e.target.value)} required />
</div>
...
{eventos.length === 0 ? (
<p style={{ marginTop: '15px', color: '#666' }}>Nenhuma partida
cadastrada.</p>
) : (
<ul style={{ listStyle: 'none', marginTop: '15px' }}>
{eventos.slice().reverse().map((evento) => (
<li key={evento.id} style={{ borderBottom: '1px solid #eee', paddingBottom:
'15px', marginBottom: '10px' }}>
<strong>{evento.timeA} x {evento.timeB}</strong> - {evento.data}
{evento.status === 'encerrado' && <span style={{ marginLeft: '10px', color:
'#e74c3c', fontSize: '12px', fontWeight: 'bold' }}>(ENCERRADO)</span>}
<br/>
<small style={{ color: '#666' }}>
Odds: Casa ({evento.odds.vitoriaA}) | Empate ({evento.odds.empate}) |

Fora ({evento.odds.vitoriaB})
</small>

{/* Botões de encerramento só aparecem se o evento estiver aberto */}
{evento.status === 'aberto' && (
<div style={{ marginTop: '10px', display: 'flex', gap: '10px' }}>
<button onClick={() => encerrarEvento(evento.id, 'vitoriaA')} style={{

background: '#2ecc71', padding: '5px 10px', fontSize: '12px' }}>Vitória
{evento.timeA}</button>

<button onClick={() => encerrarEvento(evento.id, 'empate')} style={{
background: '#f1c40f', color: '#000', padding: '5px 10px', fontSize: '12px'
}}>Empate</button>

<button onClick={() => encerrarEvento(evento.id, 'vitoriaB')} style={{

background: '#3498db', padding: '5px 10px', fontSize: '12px' }}>Vitória
{evento.timeB}</button>
</div>
)}
</li>
...
{apostas.length === 0 ? (
<p style={{ marginTop: '15px', color: '#666' }}>Nenhuma aposta feita
ainda.</p>
) : (
<ul style={{ listStyle: 'none', marginTop: '15px' }}>
{apostas.slice().reverse().map((aposta) => (
<li key={aposta.id} style={{ borderBottom: '1px solid #eee',
paddingBottom: '10px', marginBottom: '10px' }}>
Aposta ID: <strong>{aposta.id}</strong> (Feita pelo Usuário ID:
{aposta.usuarioId})<br/>

<span style={{ color: '#2980b9' }}>
Palpite: <strong>{aposta.palpite}</strong> | Valor: R$ {(aposta.valor ||

0).toFixed(2)} | Potencial: R$ {(aposta.retornoPotencial || 0).toFixed(2)}
</span>
<br/>

<small style={{ color: aposta.status === 'ganhou' ? '#27ae60' :
aposta.status === 'perdeu' ? '#e74c3c' : '#e67e22', fontWeight: 'bold' }}>
